<template>
  ...
  <div>
    <Button
      label="Adding ML-Expert"
      class="p-button-rounded p-button-success mb-3"
      @click="openMLExpertDialog()"
    />
  </div>
  <div>
    <DataTable
    ...
    >
      <template #header>
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="m-0">ML-Expert Manage</h5>
          ...
          <template #body="{ data }">
            <div class="d-flex">
              <Button
                ...
                @click="DeleteMLExpert(data.id)"
              />
              <Button
                label="Restore"
                class="p-button-raised"
                :class="{ disable: data.user.isDelete === false }"
                @click="RestoreMLExpert(data.id)"
              />
            </div>
          </template>
        </Column>
  ...
  <Dialog
    :header="
      selectedUser != null
        ? 'Edit ML-Expert Information'
        : 'Adding New ML-Expert'
    "
    v-model:visible="displayMLExpertDialog"
  ...
  >
    ...
    <div class="flex-column">
      <div
        class="p-inputgroup"
        :class="{ error: validation.hasError('user.MLExpert.experience') }"
      >
        <span class="p-inputgroup-addon">
          <i class="fa-solid fa-globe"></i>
        </span>
        <InputNumber
          ...
          v-model="user.MLExpert.experience"
        />
      </div>
      <small style="color: red">{{
        validation.firstError("user.MLExpert.experience")
      }}</small>
    </div>
    <template #footer>
      <Button
        ...
        icon="pi pi-times"
        @click="closeMLExpertDialog()"
      />
      <Button
        label="Submit"
        icon="pi pi-check"
        @click="CreateMLExpert()"
        autofocus
      />
    </template>
  </Dialog>
...
export default {
  ...
  validators: {
    ...
    "user.MLExpert.experience": function (value) {
      return Validator.value(value)
  ...
  methods: {
    ...
    openMLExpertDialog() {
      this.displayMLExpertDialog = true;
    },
    closeMLExpertDialog() {
      this.displayMLExpertDialog = false;
      this.clearform();
      this.validation.reset();
    },
    async CreateMLExpert() {
      const success = await this.$validate();
      if (success === true) {
        this.loader = true;
        HTTP.post("MLExpert", this.user)
          .then((res) => {
            this.$toast.add({
              severity: "success",
              summary: "Success Message",
              detail: "New MLExpert has create successfully",
              life: 3000,
            });
            this.loader = false;
            this.getAllMLExpert();
            this.closeMLExpertDialog();
            this.validation.reset();
            this.clearform();
          })
          .catch((error) => {
    ...
    DeleteMLExpert(id) {
      this.$confirm.require({
        ...
        accept: async () => {
          HTTP.put("MLExpert/DeleteMLExpert/" + id)
            .then((res) => {
              this.$toast.add({
                severity: "success",
                summary: "Confirmed",
    ...
    RestoreMLExpert(id) {
      this.$confirm.require({
        ...
        accept: async () => {
          HTTP.put("MLExpert/RestoreMLExpert/" + id)
            .then((res) => {
              this.$toast.add({
                severity: "success",
                summary: "Confirmed",
