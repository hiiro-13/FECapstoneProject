<template>
  ...
  >
    ...
    <div class="flex-column">
      <div
        class="p-inputgroup"
        :class="{ error: validation.hasError('user.nurse.experience') }"
      >
        <span class="p-inputgroup-addon">
          <i class="fa-solid fa-globe"></i>
        </span>
      ...
      <small style="color: red">{{
        validation.firstError("user.nurse.experience")
      }}</small>
    </div>
    <template #footer>
      <Button
...
export default {
  ...
  validators: {
    ...
    "user.nurse.experience": function (value) {
      return Validator.value(value)
  ...
  beforeUpdate() {
    if (this.selectedUser != null) {
      ...
      this.user.identityCard = this.selectedUser.identityCard;
        ...
        HTTP.post("Nurse", this.user)
          .then((res) => {
            ...
            this.$toast.add({
              ...
              detail: "New Nurse has create successfully",
              life: 3000,
            });
          })
          .catch((error) => {
